...
const Select = ({ tp, value, list, call, id, option, item, index, label }) =>
    <select aria-label={label} className='form-control' id={id + tp}
        value={value}
...
export const utInputDate = _this => ({ option, item, id, index, yearStart = 20 }) => {
    ...
    return (
        <span className='day-picker'>
            <Select {...{
                tp: 'Day', list: listDay, value: dd[2],
                label: _field.label + '_day',
                ..._props
            }} />
            <Select {...{
                tp: 'Month', list: listMonth, value: dd[1],
                label: _field.label + '_month',
                ..._props
            }} />
            <Select {...{
                tp: 'Year', list: listYear, value: dd[0],
