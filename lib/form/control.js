...
const utControl = _this => props => {
  ...
  switch (type) {
    ...
    case 'numeric':
      ...
      const matchDec = _f == null || (_f$valid = _f.valid) == null || (_f$valid = _f$valid.pattern) == null ? void 0 : _f$valid.match(/numeric field with at most (\d+) decimal/);
      ...
      if (_f.format) {
        ...
        if (_l >= 2) {
          ...
          if (_l >= 3) {
            ...
            numWidth = numWidth + Math.trunc((numWidth - (matchDec == null ? void 0 : matchDec[1]) - 0.1) / 3); // parseInt(_n[2]) not always show decimal places in program codes
