...
const _call_api = _r => tp => (func, payload, next) => {
  if (process.env.API_STATUS === 'true' && tp === 'api') {
    ...
    storeCall('call', pre_apiStatus + 'apiStatus', {
      _api_status: 'waiting'
    }, () => storeCall(tp, func, payload, res => storeCall('call', pre_apiStatus + 'apiStatus', (0, _extends2.default)({}, res.payload, {
      _api_status: 'isReady'
...
const utCreateElement = (mod, init) => {
  ...
  const _call = _call_api(_r); //** const _reducer = (state = _r.init, action) => {
